...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/Version-4.4.0-2563eb?style=for-the-badge">
...
📖 Documentation
    ...
    🔐 [Authentication](docs/authentication.md) — auth, token refresh and external redirects.
    ...
    🛡️ [Security](docs/architecture.md) — security and production recommendations.
...
Key Features
    ...
    🚀 Smart Redirects: Intelligent redirection mechanism with `destination` parameter support, handling both logged-in sessions and automatic authentication.
...
The application supports a special `/redirect` route that helps users jump directly to specific games within the ecosystem.

- **Usage:** `/app/redirect?destination=GameName`
- **Behavior:**
    - If the user is already logged in, it instantly redirects to the game's URL.
    - If not logged in, it shows the login form and performs the redirect after successful authentication.
    - Supports both user-specific games and public fallback games.
    - Case-insensitive destination matching.
    - Uses local cache (`localStorage`) for faster transitions.
