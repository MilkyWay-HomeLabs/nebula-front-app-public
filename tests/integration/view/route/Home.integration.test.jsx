...
beforeEach(() => {
    // Cookie-based session: on boot Home probes the server with UserDataRequest.
    // Default to "no active session" so the login form is shown; individual tests
    // override this to simulate a valid session cookie.
    UserDataRequest.default.mockReset();
    UserDataRequest.default.mockResolvedValue(null);
});
...
describe('Home - Integration Tests', () => {
    test('full login flow: authenticates, fetches user data, shows menu and games', async () => {
        ...
        UserDataRequest.default.mockReset();
        UserDataRequest.default.mockResolvedValueOnce(null); // boot probe: no session yet
        UserDataRequest.default.mockResolvedValue({id: 5, login: 'integuser'}); // after login
